<?php
	...
	if(isset($_POST["book_author"])){
		...
		$sql = "INSERT INTO oncology(category, book_author, book_title, book_edition, year_published, publisher, quantity, availability, borrower) VALUES(:category, :book_author, :book_title, :book_edition, :year_published, :publisher, :quantity, :availability, :borrower)";
		
		try {
			$result = $conn->prepare($sql);
			$values = array(":category" => $category, ":book_author" => $book_author, ":book_title" => $book_title, ":book_edition" => $book_edition, ":year_published" => $year_published, ":publisher" => $publisher, ":quantity" => $quantity, ":availability" => $availability, ":borrower" => $borrower);
			$result->execute($values);
			
			if($result){
				echo "<script> alert('Record has been saved!'); window.location = 'onco_neo_medicine.php'; </script>";
			}
...
<div class = "container">
	<a class = "btn btn-outline-primary" href="onco_neo_medicine.php" role="button">ONCOLOGY & NEOPLASTIC BOOKS</a>
